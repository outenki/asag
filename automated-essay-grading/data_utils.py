...
def load_glove(token_num=6, dim=50, skip_rows=1):
    word2vec = []
    word_idx = {}
    # first word is nil
    word2vec.append([0]*dim)
    count = 1
    f_glove = os.path.join(os.path.dirname(os.path.realpath(__file__)), "glove/glove.{}B.{}d.txt".format(token_num, dim))
    with open(f_glove, encoding='utf-8') as f:
    ...
    return word_idx, np.array(word2vec)
