...
class MemN2N_KV(object):
    ...
    def __init__(self, batch_size, vocab_size,
        ...
        query_size: largest number of words in sentences (including answers and memmories)
        story_size: largest number of words in story (same to query_size, used for encoding essays)
        embedding_size: The size of the word embedding. (dimension of word vectors)
        ...
        # embedding words in essays into word vectors
        self.embedded_chars = tf.nn.embedding_lookup(self.W, self._query)
        # shape: [batch_size, memory_size, story_size, embedding_size]
        # memory_key is a list of [memories] * batch_size ?
        self.mkeys_embedded_chars = tf.nn.embedding_lookup(self.W_memory, self._memory_key)
        # use the sum of word embeddings(after position encoding) as the essay embedding
        # This two variable are used for memory addressing
        # shape: [batch_size, embedding_size]
        embedded_query = tf.reduce_sum(self.embedded_chars*self._encoding, 1)
        # shape: [batch_size, memory_size, embedding_size]
        embedded_mem = tf.reduce_sum(self.mkeys_embedded_chars*self._encoding, 2)
        ...
        # TODO: key addressing
        o = self._key_addressing(embedded_mem, embedded_mem, embedded_query, r_list)
        o = tf.transpose(o)
        if reader == 'bow':
            #self.B = self.A
        ...
        with tf.name_scope("prediction"):
            ...
            # cross_entropy = tf.losses.mean_squared_error(predictions=probs, labels=tf.cast(self._score_encoding, tf.float32))
    ...
    !!!! mvalues are not used at all !!!!
    !!!! mkeys are embedded memories !!!!
    '''
