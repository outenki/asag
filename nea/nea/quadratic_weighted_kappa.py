...
def confusion_matrix(rater_a, rater_b, min_rating=None, max_rating=None):
    ...
    # modified by WANG 0222
    # add round() outside min() and max()
    if min_rating is None:
        min_rating = round(min(rater_a + rater_b))
    if max_rating is None:
        max_rating = round(max(rater_a + rater_b))
    # modified by WANG 0222
    num_ratings = int(max_rating - min_rating + 1)
    conf_mat = [[0 for i in range(num_ratings)]
                for j in range(num_ratings)]
    # modified by WANG 0222
    # add np.rint().astype(np.integer)
    for a, b in zip(np.rint(rater_a).astype(np.integer), np.rint(rater_b).astype(np.integer)):
...
def histogram(ratings, min_rating=None, max_rating=None):
    ...
    # modified by WANG 0222
    # add round() outside min() and max()
    ratings = np.rint(ratings).astype(np.integer)
    if min_rating is None:
        min_rating = round(min(ratings))
    if max_rating is None:
...
def quadratic_weighted_kappa(rater_a, rater_b, min_rating=None, max_rating=None):
    ...
    assert(len(rater_a) == len(rater_b))
    # modified by WANG 0222
    # added round() outside min and mzx
    if min_rating is None:
        min_rating = round(min(min(rater_a), min(rater_b)))
    if max_rating is None:
        max_rating = round(max(max(rater_a), max(rater_b)))
    # modified by WANG 0222
    ...
    return 1.0 - numerator / denominator if denominator != 0 else 1
