...
path_out = path_data + '_result_' + dt.strftime('%Y%m%d%H%M%S')
...
def run_shell(path_i, path_o, emb, pmt_id):
    ...
    cmd="MKL_THREADING_LAYER=GNU KERAS_BACKEND='theano' THEANO_FLAGS='device=cuda%s,floatX=float32' python train_nea.py -tr %s/train.tsv -tu %s/dev.tsv -ts %s/test.tsv -p %s -o %s -e %d" % (args.gpu, path_i, path_i, path_i, pmt_id, path_o, args.dim)
    if emb:
        cmd += ' --emb %s' % emb
    print('cmd: %s' % cmd)
    os.system(cmd)
